...
public class SomaTotal {
	...
	public int somaTotal(BSTNode<Integer> node) {
		int soma = 0;
		if(node != null && !node.isEmpty()) {
			if(!node.getLeft().isEmpty()) {
				soma += somaTotal(node.getLeft());
			}
			if(!node.getRight().isEmpty()) {
				soma += somaTotal(node.getRight());
			}
			soma += node.getData();
		}
		return soma;
	...
	public static void main(String[] args) {
		...
		SomaTotal e = new SomaTotal();
		int ancestral = e.somaTotal(n1); // 9
