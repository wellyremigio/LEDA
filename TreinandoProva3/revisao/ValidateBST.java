...
public class ValidateBST{
	...
	public boolean isValid(BSTNode<Integer> root) {
		return recursive(root, -10000, 10000);
	}
	
	private boolean recursive(BSTNode<Integer> node, Integer min, Integer max) {
		if(node == null) {
			return true;
		}
		if(!node.isEmpty() && (node.getData() >= max || node.getData() <= min)) {
			return false;
		}
		
		return recursive(node.getLeft(), min, node.getData()) && recursive(node.getRight(), node.getData(), max);
	}
	...
	public static void main(String[] args) {
		...
		ValidateBST e = new ValidateBST();;
		System.out.println(e.isValid(n1));
	}
}
