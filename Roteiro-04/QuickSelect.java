...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		if(array.length > 0 && k <= array.length && k >= 1 && array != null) {
	...
	private T quickSelect(T[] array, int k, int leftIndex, int rightIndex) {
		if(leftIndex < rightIndex && leftIndex >= 0 && rightIndex < array.length && array != null) {
			...
			if(k < pivotIndex + 1) {
				return quickSelect(array, k, leftIndex, pivotIndex-1);
				
			}else if (k > pivotIndex + 1) {
				return quickSelect(array, k, pivotIndex+1, rightIndex);
			}
	...
	private int partition(T[] array, int leftIndex, int rightIndex) {
		int indexPivot = medianaDeTres(array, leftIndex, rightIndex);
	...
	private int medianaDeTres(T[] array, int leftIndex, int rightIndex) {
