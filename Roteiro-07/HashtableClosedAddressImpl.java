...
public class HashtableClosedAddressImpl<T> extends
	...
	public T search(T element) {
		...
		if(element != null && !isEmpty()) {
			int indice = indexOf(element);
			if(indice != -1) {
				int indiceInterno = ((LinkedList<T>) table[indice]).indexOf(element);
				resposta = ((LinkedList<T>) table[indice]).get(indiceInterno);
			}
		}
		return resposta;
	}
	...
	public int indexOf(T element) {
		...
		if(element != null && !isEmpty()) {
			int indice = ((HashFunctionClosedAddress<T>) this.hashFunction).hash(element);
			LinkedList<T> slot = (LinkedList<T>) table[indice];
